...
// ============================================================================
// AmplitudeExtractor implementation
// ============================================================================
...
// ============================================================================
// PanningExtractor implementation
// ============================================================================
...
// ============================================================================
// SpectralExtractor implementation
// ============================================================================
...
// ============================================================================
// TransientExtractor implementation
// ============================================================================
...
// ============================================================================
// ADSREnvelopeExtractor Implementation
// ============================================================================

std::vector<std::vector<std::pair<double, double>>> ADSREnvelopeExtractor::extract(const juce::AudioBuffer<float>& buffer,
    double sampleRate,
    int channel) {

    std::vector<std::pair<double, double>> amplitudeData;
    ...
    for (int start = 0; start < numSamples; start += hopSamples) {
        ...
        float rms = std::sqrt(sumSquares / length);
        amplitudeData.push_back({ start / sampleRate, rms });
    }

    if (!amplitudeData.empty() && settings.normalizeOutput) {
        float maxVal = 0.0f;
        for (const auto& [time, value] : amplitudeData) {
            maxVal = std::max(maxVal, static_cast<float>(value));
        }
        if (maxVal > 0.0f) {
            for (auto& [time, value] : amplitudeData) {
                value /= maxVal;
            }
        }
    }

    return extractFromAmplitude(amplitudeData, sampleRate);
}

std::vector<std::vector<std::pair<double, double>>> ADSREnvelopeExtractor::extractFromAmplitude(
    const std::vector<std::pair<double, double>>& amplitudeData,
    double sampleRate) {

    std::vector<std::vector<std::pair<double, double>>> results(5);

    if (amplitudeData.size() < 10) return results;

    auto params = detectADSR(amplitudeData);
    double totalTime = amplitudeData.empty() ? 5.0 : amplitudeData.back().first;

    results[0].push_back({ 0.0, params.attackTime });
    results[0].push_back({ totalTime, params.attackTime });

    results[1].push_back({ 0.0, params.decayTime });
    results[1].push_back({ totalTime, params.decayTime });

    results[2].push_back({ 0.0, params.sustainLevel });
    results[2].push_back({ totalTime, params.sustainLevel });

    results[3].push_back({ 0.0, params.releaseTime });
    results[3].push_back({ totalTime, params.releaseTime });

    auto envelope = generateADSREnvelope(params, totalTime);
    results[4] = envelope;

    return results;
}

ADSREnvelopeExtractor::ADSRParams ADSREnvelopeExtractor::detectADSR(
    const std::vector<std::pair<double, double>>& curve) {

    ADSRParams params;

    if (curve.empty()) return params;

    params.peakValue = 0.0;
    for (const auto& [time, value] : curve) {
        if (value > params.peakValue) {
            params.peakValue = value;
            params.peakTime = time;
        }
    }

    if (params.peakValue < 0.001) return params;

    double attackStartThreshold = params.peakValue * 0.1;
    double attackEndThreshold = params.peakValue * 0.9;

    double attackStartTime = 0.0;
    double attackEndTime = params.peakTime;

    for (const auto& [time, value] : curve) {
        if (value >= attackStartThreshold) {
    ...
    for (const auto& [time, value] : curve) {
        if (value >= attackEndThreshold) {
            attackEndTime = time;
            break;
        }
    }

    params.attackTime = std::max(0.001, attackEndTime - attackStartTime);

    size_t middleStart = static_cast<size_t>(curve.size() * 0.3);
    size_t middleEnd = static_cast<size_t>(curve.size() * 0.7);

    if (middleEnd > curve.size()) middleEnd = curve.size();
    if (middleStart >= middleEnd) middleStart = middleEnd / 2;

    std::vector<double> middleValues;
    for (size_t i = middleStart; i < middleEnd && i < curve.size(); ++i) {
        middleValues.push_back(curve[i].second);
    }

    if (!middleValues.empty()) {
        std::sort(middleValues.begin(), middleValues.end());
        double medianValue = middleValues[middleValues.size() / 2];
        params.sustainLevel = medianValue / params.peakValue;
        params.sustainLevel = juce::jlimit(0.01, 0.99, params.sustainLevel);
    }

    double sustainThreshold = params.peakValue * params.sustainLevel;
    double decayEndTime = params.peakTime;

    for (const auto& [time, value] : curve) {
        if (time > params.peakTime && value <= sustainThreshold) {
            decayEndTime = time;
            break;
        }
    }

    params.decayTime = std::max(0.0, decayEndTime - params.peakTime);

    double releaseThreshold = params.peakValue * 0.1;
    params.releaseStartTime = curve.back().first;

    double releaseStartThreshold = params.sustainLevel * params.peakValue * 0.8;
    for (int i = static_cast<int>(curve.size()) - 1; i >= 0; --i) {
        if (curve[i].second >= releaseStartThreshold) {
            params.releaseStartTime = curve[i].first;
            break;
        }
    }

    double releaseEndTime = curve.back().first;
    for (const auto& [time, value] : curve) {
        if (time >= params.releaseStartTime && value <= releaseThreshold) {
            releaseEndTime = time;
            break;
        }
    }

    params.releaseTime = std::max(0.0, releaseEndTime - params.releaseStartTime);

    params.attackTime = juce::jlimit(0.001, 10.0, params.attackTime);
    params.decayTime = juce::jlimit(0.001, 10.0, params.decayTime);
    params.releaseTime = juce::jlimit(0.001, 10.0, params.releaseTime);
    params.sustainLevel = juce::jlimit(0.01, 0.99, params.sustainLevel);

    return params;
}

std::vector<std::pair<double, double>> ADSREnvelopeExtractor::generateADSREnvelope(
    const ADSRParams& params, double totalDuration) {

    std::vector<std::pair<double, double>> envelope;

    if (totalDuration <= 0.0) return envelope;

    int numPoints = 500;
    double timeStep = totalDuration / numPoints;
    ...
    for (int i = 0; i <= numPoints; ++i) {
        ...
        if (t < params.attackTime) {
            value = (t / params.attackTime);
        }
        else if (t < params.attackTime + params.decayTime) {
            double decayProgress = (t - params.attackTime) / params.decayTime;
            value = 1.0 - (1.0 - params.sustainLevel) * decayProgress;
        }
        else if (t < params.releaseStartTime) {
            value = params.sustainLevel;
        }
        else if (t < params.releaseStartTime + params.releaseTime) {
            if (params.releaseTime > 0.0) {
                double releaseProgress = (t - params.releaseStartTime) / params.releaseTime;
                value = params.sustainLevel * (1.0 - releaseProgress);
            }
        }
        else {
            value = 0.0;
        ...
        value = juce::jlimit(0.0, 1.0, value) * params.peakValue;
        envelope.push_back({ t, value });
    }
...
std::unique_ptr<FeatureExtractor> FeatureExtractorFactory::createExtractor(const juce::String& name) {
    ...
    if (name == "ADSR Envelope") return std::make_unique<ADSREnvelopeExtractor>();
    return nullptr;
}

juce::StringArray FeatureExtractorFactory::getAvailableFeatures() {
    return juce::StringArray{ "Amplitude", "Panning", "Spectral", "Pitch", "Transients", "ADSR Envelope" };
}
...
juce::Colour FeatureExtractorFactory::getFeatureColour(const juce::String& name) {
    static std::map<juce::String, juce::Colour> colours = {
        ...
        {"ADSR Envelope", juce::Colours::cyan}
    };
